  ...
  useEffect(() => {
    ...
    fetch("/api/reports?data_type=report")
      ...
      .then((all: Report[]) => {
        ...
        if (paramId && filtered.some((r) => String(r.id) === paramId)) {
          setSelectedReportId(paramId);
          setSelectedSection("surveys");
          loadSurveys(paramId);
        }
      })
      .catch(() => {})
      .finally(() => setLoadingReports(false));
  }, [user, searchParams, loadSurveys]);
        ...
        <div className="flex items-center gap-3">
          ...
          {showContent && surveys.length > 0 && (
            saveSuccess ? (
              ...
              >
                {saving ? <><Loader2 className="size-3.5 animate-spin mr-1.5" /> Saving…</> : <><Save className="size-3.5 mr-1.5" /> Save changes</>}
              </Button>
            )
          )}
        </div>
        ...
        {!showContent ? (
          ...
          <div className="space-y-4">
            {surveys.map((survey, i) => {
              ...
              return (
                <div key={survey.id} className={cn("rounded-xl border bg-card p-5 space-y-4 transition-colors", state.dirty && "border-amber-200")}>
