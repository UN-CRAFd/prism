...
export function ProdocIndicatorsSection({
  ...
  return (
    // Stacked full-width boxes, with the add button / search box in the gap between
    // them. The section claims the tab's full remaining height and never scrolls
    // itself — the two boxes divide that height and scroll internally instead.
    <div className={"flex flex-col gap-4 " + (fillHeight ? "flex-1 min-h-0" : "")}>
      {renderTable(standardLines, "standard")}

      {/* shrink-0 so the add/search block keeps its natural height and only the
          boxes absorb the leftover space. */}
      <div className="flex shrink-0 flex-col items-start gap-2">
        ...
        <Button type="button" variant="outline" size="sm" disabled={readOnly} onClick={() => creating ? cancelCreate() : setShowPicker((open) => !open)} className="gap-1">
