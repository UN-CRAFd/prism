      ...
      <table className="text-sm border-separate border-spacing-0" style={{ minWidth: FROZEN_WIDTH }}>
        <thead>
          <tr className="text-xs">
            ...
            {visibleYears.map((y) => (
              <th key={y} colSpan={y === currentYear ? 4 : hasReport(y) ? 3 : 1} className={cn("px-2 py-2 text-center text-muted-foreground border-l border-b", HEAD_TEXT, y === currentYear ? CURRENT_YEAR_HEAD : "bg-neutral-100", fillHeight && "sticky top-0 z-30 h-8")}>{y}</th>
            ))}
          </tr>
          <tr className="text-muted-foreground">
            ...
            {visibleYears.map((y) =>
              hasReport(y)
                ? <FragmentYearHead key={y} current={y === currentYear} past={y !== currentYear} fillHeight={fillHeight} />
...
function FragmentYearHead({ current, past = false, fillHeight = false }: { current: boolean; past?: boolean; fillHeight?: boolean }) {
  ...
  return (
    <>
      ...
      {!past && <th className={cn("px-2 py-1.5 text-left border-b min-w-[160px]", SUBHEAD_TEXT, current ? CURRENT_YEAR_HEAD : "bg-neutral-50", sticky)}>Comment</th>}
    </>
  );
}
...
function YearCells({
  editable, past = false, approved, exp, diff, expInput, comment, onExp, onComment,
}: {
  editable: boolean;
  past?: boolean;
...
function FooterYearCells({ approved, exp, strong, past = false }: { approved: number; exp: number; strong?: boolean; past?: boolean }) {
  return (
    <>
      ...
      {!past && <td className="px-2 py-2 border-t" />}
    </>
  );
}
...
export function ExpenditureAdminEditor({ projectId, isAdmin = true, fillHeight = false, onSaveStateChange }: { projectId: number; isAdmin?: boolean; fillHeight?: boolean; onSaveStateChange?: (s: SaveState) => void }) {
  ...
  const [categoryNotes, setCategoryNotes] = useState<Record<number, string>>({}); // catId → description
  ...
  const dirtyCatNotesRef = useRef<Set<number>>(new Set());
  const amountsRef = useRef<Record<string, string>>({});
  const categoryNotesRef = useRef<Record<number, string>>({});
  const timerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const savingRef = useRef(false);
  const flushRef = useRef<() => void>(() => {});

  useEffect(() => { amountsRef.current = amounts; }, [amounts]);
  useEffect(() => { categoryNotesRef.current = categoryNotes; }, [categoryNotes]);
  ...
  const flush = async () => {
    ...
    setSaveState("saving");
    try {
      const amountKeys = Array.from(dirtyAmountsRef.current);
      dirtyAmountsRef.current.clear();
      const noteCatIds = Array.from(dirtyCatNotesRef.current);
      dirtyCatNotesRef.current.clear();
      ...
      for (const catId of noteCatIds) {
        const note = (categoryNotesRef.current[catId] ?? "").trim();
        const res = await fetch("/api/expenditure-budgets", {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  function setCategoryNote(catId: number, v: string) {
    setCategoryNotes((prev) => ({ ...prev, [catId]: v }));
    dirtyCatNotesRef.current.add(catId);
    scheduleFlush();
  }
